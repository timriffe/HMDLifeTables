#' @title  Aggregate an age by year matrix \code{YearAgg} into N-year groups
...
YearAgg <- function(mat, years = as.integer(colnames(mat)), N = 1){
    ...
    yr0       <- as.integer(colnames(out))  # label columns to indicate ranges, same as unique(fac)
    #yrlast    <- sort(unique(c(years[years %% N == (N - 1)], max(years))))
    yrlast <- c( yr0[-1] -1, max(years))
    colnames(out) <- paste(yr0, yrlast, sep = "-" )
    # this will remove 1 year groupings from 5 or 10 year aggregations
    is.singleton <- as.vector( table(fac) < 2 )
    #out       <- out[, yr0 != yrlast]
    out <- out[, ! is.singleton]
    return(out)
